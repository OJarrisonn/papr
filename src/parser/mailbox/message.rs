...
mod tests {
    ...
    fn test_message_try_from_git() {
        ...
        assert_eq!(
            message.headers[0],
            ("Subject", "[PATCH v1 1/10] patch-tree: foo message")
                .try_into()
                .unwrap()
        );
        assert_eq!(
            message.headers[1],
            ("From", "John Doe <\"john.doe at email.com\">")
                .try_into()
                .unwrap()
        );
        assert_eq!(
            message.headers[2],
            ("Date", "Wed, 08 Jun 2022 12:00:01 -0300")
                .try_into()
                .unwrap()
        );
        assert_eq!(
            message.headers[3],
            ("Message-Id", "<20220608-john-doe@email.com>")
                .try_into()
                .unwrap()
        );
        assert_eq!(
            message.headers[4],
            ("MIME-Version", "1.0").try_into().unwrap()
        );
        assert_eq!(
            message.headers[5],
        ...
        assert_eq!(
            message.headers[6],
            ("Content-Transfer-Encoding", "7bit").try_into().unwrap()
        );

        let break_point = include_str!("samples/single_patch.mbx")
            .find("\n\n")
